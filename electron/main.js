...
function startBackend() {
  ...
  backendProcess.stdout?.on("data", (d) => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send("backend-log", { type: "info", text: logMsg });
    }
  });
  ...
  backendProcess.stderr?.on("data", (d) => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send("backend-log", { type: "error", text: errorMsg });
    }
  });
...
ipcMain.handle("trim-multiple-segments", async (event, { inputPath, segments }) => {
  try {
    ...
    const tasks = segments.map((seg, index) => async () => {
      ...
      const args = ["-y", "-ss", seg.startTime.toString(), "-t", seg.duration.toString(), "-i", inputPath, "-c", "copy", "-map", "0", "-movflags", "+faststart", outPath];
